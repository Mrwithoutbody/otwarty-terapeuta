...
describe('formularz panelu: tożsamość tak, treść nie', () => {
  ...
  it('nie pokazuje pól treści, za to prowadzi do edytora strony', async () => {
    ...
    // Cennik i FAQ też żyją w edytorze strony.
    expect(html).not.toContain('data-tab-label="Oferta"');
    expect(html).not.toContain('data-tab-label="FAQ"');
    // Przycisk stoi w „Dane”, a dialog w ukrytym panelu nie otworzyłby się: okno leży za zakładkami.
